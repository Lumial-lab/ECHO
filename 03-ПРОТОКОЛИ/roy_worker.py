...
SYSTEM = (
    ...
    "Тобі дають ОДИН вузол дослідницького графа: твердження або питання. "
    "Дай стислий, чесний, перевірюваний результат УКРАЇНСЬКОЮ: "
    "1) відповідь по суті (без води), 2) хід міркування коротко, "
    "3) що САМЕ треба перевірити емпірично, щоб прийняти результат "
    "(конкретні команди/джерела/тести), 4) відверті межі й ризики. "
    "Якщо задача потребує запуску коду чи доступу, якого не маєш — скажи прямо "
    "і сформулюй мінімальний стенд. НЕ вигадуй фактів і цифр."
)
...
def run_once(dag: str, dry: bool = False) -> int:
    ...
    for n in batch:
        ...
        prompt = (f"Вузол {n['id']} ({n['kind']}): {n['title']}\n\n"
                  f"Опис/контекст: {n['desc'].replace('[auto]', '').strip()}\n"
                  + (f"Нотатки: {' | '.join(n['notes'][-3:])}\n" if n["notes"] else "")
                  + (f"Минулі невдалі спроби (НЕ повторюй їх шлях): "
