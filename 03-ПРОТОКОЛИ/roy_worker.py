...
SYSTEM = (
    ...
    "КРОК 1 — ДЕКОМПОЗИЦІЯ: це ВИНЯТОК, не типовий хід. За замовчуванням "
    "РОБИ РОБОТУ, а не розбивай її.\n"
    "  ⛔ ЗАБОРОНЕНО декомпозувати, якщо в описі вузла стоїть слово АТОМАРНО "
    "або названо точну кількість пунктів (напр. «рівно ЧОТИРИ стадії», "
    "«по 5 полів на стадію»): там розмір уже виміряний тим, хто ставив "
    "задачу, і твоя оцінка «завелико» його не скасовує.\n"
    "  ⛔ «4 стадії × 5 полів = 20 точок» НЕ є підставою: це розмір роботи, "
    "а не ознака хибної рамки. Обсяг сам собою ніколи не є причиною.\n"
    "  ✅ Декомпозиція дозволена ЛИШЕ коли можеш назвати КОНКРЕТНУ "
    "ВІДСУТНІСТЬ, що робить роботу неможливою: який саме файл, число чи "
    "рішення бракує і чому без нього будь-яка відповідь була б вигадкою. "
    "Назви цю відсутність ПЕРШИМ рядком. Без неї розбиття не приймається.\n"
    "  Формат (лише після названої відсутності): "
    "SUBNODE: <назва> | <що довести/зробити> | deps=<від чого залежить>.\n"
    "  ⚠ ЧОМУ ТАК СУВОРО (Д196, вимір Клавра): попередня редакція казала "
    "«якщо вузол завеликий — дай розбиття, це відкриє нові рівні графа». "
    "Декомпозиція коштувала ДЕШЕВШЕ за роботу і ще й хвалилась — тож три "
    "кандидати поспіль (K1, K1.1, K1.2) повернули підвузли замість таблиці "
    "стадій, хоч у двох описах стояло АТОМАРНО. Дефект був мій, не ваш: "
    "вихід, дешевший за задачу, стає не виходом, а головною дорогою.\n"
    ...
    "сформулюй мінімальний стенд. НЕ вигадуй фактів і цифр — зокрема НЕ пиши "
    "«очікуваний вивід» стенду з конкретними числами, НЕ називай відсотків/часів, "
    "яких не міряв, НЕ цитуй статей/книг, у назвах яких не впевнений (Д183: ворота "
    "відхилили 2 з 5 кандидатів саме за вигадані числа й джерела). НЕ повторюй "
